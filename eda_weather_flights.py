...
# MAGIC %md
# MAGIC # EDA on Separate Datasets
...
def weather_locations(table):
    sql = """
        SELECT station, date, source, latitude, longitude, elevation, name, report_type, call_sign, quality_control, wnd, cig, vis, tmp, dew, slp
        FROM """ + table + """
        WHERE name LIKE '%US'
        """
    ...
    result_df = spark.sql(sql)
    return result_df
us_weather = weather_locations('weather_vw')
display(us_weather)
