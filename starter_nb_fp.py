...
# MAGIC 
# MAGIC 
# MAGIC > 1. Explain the data: simple exploratory analysis of various field, such as the semantic as well as intrinsic meaning of ranges, null values, categorical/numerical, mean/std.dev to normalize and/or scale inputs.  Identify any missing or corrupt (i.e. outlier) data.
# MAGIC > 2. Define the outcome (i.e., the evaluation metric and the target) precisely, including mathematical formulas.
# MAGIC > 3. How do you ingest the CSV files and represent them efficiently? (thing about different file formats)
...
# MAGIC > 6. Split the data in train/validation/test - making sure that no leaks occur, for example: normalize using training statistics.
# MAGIC    a. `HINT`: see [this](https://hub.packtpub.com/cross-validation-strategies-for-time-series-forecasting-tutorial/) and [this](https://medium.com/@soumyachess1496/cross-validation-in-time-series-566ae4981ce4#:~:text=Cross%20Validation%20on%20Time%20Series,for%20the%20forecasted%20data%20points).
...
def test():
  df = spark.read.parquet(CLEAN_WEATHER_LOC)
  display(df)
